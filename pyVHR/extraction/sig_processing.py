...
class SignalProcessing():
    ...
    def set_landmarks(self, landmarks_list):
        ...
        if not isinstance(landmarks_list, list):
            ...
            return
        if landmarks_list[0] < 0 or landmarks_list[0] > 467:
            print("[ERROR] landmarks_set must contain positive integers between 0 and 467!")
            return
    ...
    # Modified
    def set_landmark_roi(self, landmarks_list, sampling_method, nb_sample_points):
        """
        Set the landmarks to use as ROI. 
        ...
        Args:
            landmarks_list (list): list of landmarks names to use (ex: ['glabella', 'soft triangle']) that identify patches.
            sampling_method (str): 'all' or 'random'. If 'random', it samples nb_sample_points points from the inside of the patch.
        ...
        if sampling_method != 'all' and nb_sample_points < 0:  
            print("[ERROR] Invalid sampling method or number of points")
            return
        self.ldmks = landmarks_list
        self.sampling_method = sampling_method
        self.nb_sample_points = nb_sample_points

    def extract_landmarks(self, videoFileName):
        """
        This method compute the RGB-mean signal using specific landmarks (ex: glabella).
        ...
        if depth(self.ldmks) != 2:
            print("[ERROR] For landmark shaped patches, the list of landmarks must be 2D (e.g. [[1,2,3],[4,5,6]])")
            return None
        ...
        with mp_face_mesh.FaceMesh(
            ...
            for frame in extract_frames_yield(videoFileName):
                ...
                for region in self.ldmks:
                    ...
                    magic_ldmks.append(keypoints)  # coordinates of MediaPipe keypoints for each selected landmark
                temp = np.empty((len(magic_ldmks),5))
                for i,patch in enumerate(magic_ldmks):
                    temp[i,:] = sig_ext_met(patch, full_skin_im, self.sampling_method, self.nb_sample_points,
                ...
                if self.visualize_landmarks == True:
                    ...
                    color = np.array([self.font_color[0],
                                    self.font_color[1], self.font_color[2]], dtype=np.uint8)
                    keypoints = np.unique(sum(self.ldmks, []))

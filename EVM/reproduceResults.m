...
dataDir = './data';
resultsDir = 'ResultsDavid';

mkdir(resultsDir);

%% test LGI-PPGI
% amplify_spatial_Gdown_temporal_ideal(vidFile,outDir,alpha,level, ...
%                    fl,fh,samplingRate, chromAttenuation)
% inFile,resultsDir,50,4, 0.5,4,25, 1
inFile = fullfile(dataDir,'cv_camera_david.avi');
...
% Define the range of parameter values for the grid search
alphas = [25, 50, 75];
levels = [2,4,6];
chromAttenuations = [0,0.5,1];
